...
def main() -> int:
    ...
    try:
        for batch in tqdm(pf.iter_batches(batch_size=_BATCH_SIZE),
            ...
            for i in range(n):
                vid = d["version_id"][i] if i < len(d["version_id"]) else None
                if not vid:
                    continue

                text = d["text"][i] if "text" in d and i < len(d["text"]) else ""
                text = text or ""
                citation = d["citation"][i] if "citation" in d and i < len(d["citation"]) else None

                # 이 데이터셋에는 title 컬럼이 없음. citation을 title로 사용.
